...
class TaskTest extends PHPUnit_Framework_TestCase
{
  ...
  public function testCustomRestRetryMapAddsNewHandlers()
  {
    $this->client->setClassConfig(
        'Google\Service\Exception',
        array('retry_map' => array('403' => \Google\Config::TASK_RETRY_ALWAYS))
    );
   ...
   * @expectedException \Google\IO\Exception
   */
  ...
  public function testCustomCurlRetryMapAddsNewHandlers()
  {
    $this->client->setClassConfig(
        'Google\IO\Exception',
        array('retry_map' => array(
            CURLE_COULDNT_RESOLVE_PROXY => \Google\Config::TASK_RETRY_ALWAYS
        ))
    );
  ...
  {
    $this->setExpectedException('Google\Task\Exception', $message);
    $this->setTaskConfig($config);

    new \Google\Task\Runner(
        $this->client,
        '',
  ...
  {
    $this->setNextTaskAllowedRetries(\Google\Config::TASK_RETRY_ALWAYS)
         ->runTask();
  }
  ...
  {
    $this->setTaskConfig(array('retries' => 1));
    $this->setNextTasksAllowedRetries(2, \Google\Config::TASK_RETRY_ALWAYS)
         ->runTask();
  }
  ...
  {
    $this->setTaskConfig(array('retries' => 5));
    $this->setNextTasksAllowedRetries(6, \Google\Config::TASK_RETRY_ALWAYS)
         ->runTask();
  }
  ...
  {
    $this->setTaskConfig(array('retries' => 1));
    $result = $this->setNextTaskAllowedRetries(\Google\Config::TASK_RETRY_ALWAYS)
  ...
  {
    $this->setTaskConfig(array('retries' => 5));
    $result = $this->setNextTasksAllowedRetries(2, \Google\Config::TASK_RETRY_ALWAYS)
                   ->setNextTaskReturnValue('success')
                   ->runTask();
  ...
  {
    return array(
        array(\Google\Config::TASK_RETRY_NEVER),
        array(\Google\Config::TASK_RETRY_ONCE),
    );
  }
  ...
  private function setTaskConfig(array $config)
  {
    ...
    $this->client->setClassConfig('Google\Task\Runner', $config);
  }
  ...
  private function setNextResponseThrows($message, $code)
  {
    ...
    $this->mockedCalls[$this->mockedCallsCount++] = new \Google\IO\Exception(
        $message,
        $code,
        null,
        $map
  ...
  {
    $request = new \Google\Http\Request('/test');

    $io = $this->getMockBuilder('Google\IO\IoAbstract')
    ...
    return \Google\Http\Rest::execute($this->client, $request);
  }

  /**
   * Gets the next mocked response.
   *
   * @param \Google\Http\Request $request The mocked request
   *
  ...
  private function runTask()
  {
    ...
    $exception = $this->getMockBuilder('Google\IO\Exception')
                      ->disableOriginalConstructor()

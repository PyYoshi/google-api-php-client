...
class IoTest extends BaseTest
{
  ...
  {
    $default = function_exists('curl_version') ? 'Google\IO\Curl' : 'Google\IO\Stream';
    ...
    $this->assertInstanceOf('Google\IO\Stream', $client->getIo());
  }

  public function testStreamSetTimeout()
  {
    $io = new \Google\IO\Stream($this->getClient());
  ...
  {
    $client = $this->getClient();
    $io = new \Google\IO\Stream($client);
  ...
  public function testCurlSetTimeout()
  {
    ...
    $io = new \Google\IO\Curl($this->getClient());
    $this->timeoutChecker($io);
  }
  ...
  public function testCacheRevalidate()
  {
    ...
    $req = new \Google\Http\Request('/test', 'GET');
    ...
    $res = $io->makeRequest(new \Google\Http\Request('/test', 'GET'));
  ...
  public function invalidRequest($io)
  {
    ...
    $req = new \Google\Http\Request($url, "GET");
    $io->makeRequest($req);
  }
  ...
  public function cacheHit($io)
  {
    ...
    $cacheReq = new \Google\Http\Request($url, "GET");
    ...
    $res = $io->makeRequest(new \Google\Http\Request($url, "GET"));
    $this->assertEquals("{\"a\": \"foo\"}", $res->getResponseBody());
    $this->assertEquals(200, $res->getResponseHttpCode());
  }
  ...
  public function responseChecker($io)
  {
    ...
    if (function_exists('curl_version')) {
      ...
      $hasQuirk = $curlVer['version_number'] < \Google\IO\Curl::NO_QUIRK_VERSION;
    }
    ...
    foreach ($connection_established_headers as $established_header) {
      ...
      // correct test result.
      if ($hasQuirk && get_class($io) === 'Google\IO\Curl') {
  ...
  {
    $req = new \Google\Http\Request("http://localhost.com");
    ...
    $this->assertEquals(
        \Google\IO\IoAbstract::FORM_URLENCODED,

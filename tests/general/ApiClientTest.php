...
class ApiClientTest extends BaseTest
{
    public function testClient()
    {
        ...
        $req = new \Google\Http\Request('http://foo.com');
    ...
    /**
     * @expectedException \Google\Auth\Exception
     */
...
//    $this->assertInstanceOf('Google\Http\Request', $dr_service->files->listFiles());
    }
    ...
    public function testSettersGetters()
    {
        ...
        $client->setAuth(new \Google\Auth\Simple($client));
        $client->setAuth(new \Google\Auth\OAuth2($client));

        try {
            $client->setAccessToken(null);
            die('Should have thrown an \Google\Auth\Exception.');
    ...
    public function testAppEngineAutoConfig()
    {
        ...
        $client = new \Google\Client();
        $this->assertInstanceOf('Google\Cache\Memcache', $client->getCache());
    ...
    public function testJsonConfig()
    {
        ...
        $cfg = $config->getClassConfig('Google\Auth\OAuth2');
    ...
    {
        $config = new \Google\Config(__DIR__ . "/testdata/test.ini");
        $this->assertEquals('My Test application', $config->getApplicationName());
        $this->assertEquals(
            'gjfiwnGinpena3',
            $config->getClassConfig('Google\Auth\OAuth2', 'client_secret')
        );
        $this->assertInternalType(
            'array',
            $config->getClassConfig('Google\IO\IoAbstract')
        );
        $this->assertEquals(
            100,
            $config->getClassConfig('Google\IO\IoAbstract', 'request_timeout_seconds')
        );
    }
    ...
    public function testServiceAccountJson()
    {
        ...
        $this->assertInstanceOf('Google\Auth\AssertionCredentials', $c);
    }

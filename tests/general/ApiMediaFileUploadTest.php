...
class ApiMediaFileUploadTest extends BaseTest
{
    public function testMediaFile()
    {
        ...
        $request = new \Google\Http\Request('http://www.example.com', 'POST');
    ...
    public function testGetUploadType()
    {
        ...
        $media = new \Google\Http\MediaFileUpload($client, $request, 'image/png', 'a', true);
        $params = array('mediaUpload' => array('value' => $media));
        $this->assertEquals('resumable', $media->getUploadType(null));

        // Test data *only* uploads
        $media = new \Google\Http\MediaFileUpload($client, $request, 'image/png', 'a', false);
    ...
    public function testProcess()
    {
        ...
        // Test data *only* uploads.
        $request = new \Google\Http\Request('http://www.example.com', 'POST');
        $media = new \Google\Http\MediaFileUpload($client, $request, 'image/png', $data, false);
        ...
        $media = new \Google\Http\MediaFileUpload($client, $request, 'image/png', $data, true);

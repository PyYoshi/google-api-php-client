...
class LoggerTest extends PHPUnit_Framework_TestCase
{
   ...
   * @expectedException \Google\Logger\Exception
  ...
  {
    $this->client->setClassConfig('Google\Logger\LoggerAbstract', 'level', $level);
    $this->getLogger();
  }
  ...
  {
    $this->client->setClassConfig('Google\Logger\LoggerAbstract', 'level', $config);
    $logger = $this->getLogger();
  ...
  {
    $logger = $this->getLogger('write', 'Google\Logger\Null');
  ...
  {
    $logger = $this->getLogger('write', 'Google\Logger\Psr');
    $logger->expects($this->never())
           ->method('write');
  ...
  {
    $logger = $this->getLogger(null, 'Google\Logger\Psr');
    $this->assertFalse($logger->shouldHandle('emergency'));
    $this->assertFalse($logger->shouldHandle(600));
  }
  ...
  {
    $this->client->setClassConfig('Google\Logger\File', 'file', false);
    $logger = $this->getLogger(null, 'Google\Logger\File');
  }
  ...
  {
    $this->client->setClassConfig('Google\Logger\File', 'file', 'not://exist');
  ...
  public function testLoggerWithClosedFileReference()
  {
    ...
    $this->client->setClassConfig('Google\Logger\File', 'file', $fp);
    $logger = $this->getLogger(null, 'Google\Logger\File');
  ...
  private function getLogger($methods = null, $type = '\Google\Logger\LoggerAbstract')
  {
